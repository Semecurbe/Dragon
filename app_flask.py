...
def api_key_status() -> tuple[str, str]:
    ...
    if provider == "gemini":
        ...
        if key:
            return "success", f"✨ Gemini API active — {model}"
        return "danger", "No Gemini API key — go to Settings to add one."
    if provider == "openai":
        key = config.get("openai_api_key", "")
        model = config.get("openai_model", "gpt-4o")
        if key:
            return "success", f"🟢 OpenAI API active — {model}"
        return "danger", "No OpenAI API key — go to Settings to add one."
    # Claude (default)
...
def _gemini_chat(messages: list[dict], system: str, model: str, api_key: str) -> str:
    try:
...
def _openai_chat(messages: list[dict], system: str, model: str, api_key: str) -> str:
    try:
...
def _fetch_neighbor_chunks(
    collection, matched_ids: list[str], neighbor: int = NEIGHBOR_CHUNKS
) -> tuple[list[str], list[dict]]:
    """Fetch up to `neighbor` chunks before and after each matched chunk.
    ...
    for chunk_id in matched_ids:
        ...
        try:
            ...
            continue
        for offset in [o for r in range(1, neighbor + 1) for o in (-r, r)]:
            n = idx + offset
            if n >= 0:
                nid = f"{source}_{n}"
                if nid not in seen:
...
def settings_save():
    ...
    if provider == "claude":
        ...
        updates["gemini_model"] = request.form.get("gemini_model", "gemini-2.0-flash").strip()
        msg = f"Gemini configured: {updates['gemini_model']}."
    elif provider == "openai":
        key = request.form.get("openai_api_key", "").strip()
        if key:
            updates["openai_api_key"] = key
        updates["openai_model"] = request.form.get("openai_model", "gpt-4o").strip()
        msg = f"OpenAI configured: {updates['openai_model']}."
    else:
    ...
    try:
        ...
        pass
    try:
        neighbor_chunks = max(0, min(5, int(request.form.get("neighbor_chunks", NEIGHBOR_CHUNKS))))
        updates["neighbor_chunks"] = neighbor_chunks
    except ValueError:
        pass
...
def api_chat():
    ...
    model_label = {
        "claude": CLAUDE_MODEL,
        "gemini": config.get("gemini_model", "gemini-2.0-flash"),
        "openai": config.get("openai_model", "gpt-4o"),
        "ollama": config.get("ollama_model", "llama3.2"),
    }.get(provider, provider)

    thinking_chunks = []
    for chunk_id, doc, meta in zip(matched_ids, docs, metas):
        headings = json.loads(meta.get("headings", "[]"))
        thinking_chunks.append({
            "source":   meta.get("source", "").removesuffix("_chunks"),
            "headings": headings,
            "text":     doc[:300],
            "type":     "match",
        })
    ...
    thinking_steps: list[dict] = [
        {"icon": "🔤", "label": "Translation",
         "detail": f"🇬🇧 {question_en}  ·  🇫🇷 {question_fr}"},
        {"icon": "🔍", "label": "Vector search",
         "detail": (f"{len(matched_ids)} chunk{'s' if len(matched_ids) != 1 else ''} "
                    f"retrieved (top {n_results} per query, EN + FR merged)")},
    ]
    if neighbor_chunks > 0:
        thinking_steps.append({
            "icon": "📎", "label": "Context expansion",
            "detail": (f"{len(neighbor_docs)} neighbor chunk{'s' if len(neighbor_docs) != 1 else ''} "
                       f"added (±{neighbor_chunks} around each match)"),
        })
    thinking_steps.append({
        "icon": "🤖", "label": "Model",
        "detail": f"{provider} — {model_label}",
    })
